<?php
	...
	if(isset($_POST['submit'])){
		...
		$sql_store="INSERT into contact (name, subject, email, message) VALUES ('$name','$subject','$email','$message')";
		
		$result=mysqli_query($db,$sql_store);
		if($result){
			echo "<script>if(confirm('Thank you for your comment!')){document.location.href='index.php'};</script>";
		} else {
			echo "<script>if(confirm('Cannot add to database')){document.location.href='contact.php'};</script>";
			return;
		}
