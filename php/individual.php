<?php 
	...
	if(isset($_POST['submit'])){
		...
		$sql_store="INSERT into individual (username, firstname, lastname, email, password, phone) VALUES ('$username','$firstname','$lastname','$email','$password','$phone')";
		...
		$sql_fetch_email = "SELECT email FROM individual WHERE email= '$email'";
		...
		if(preg_match('/[\'^£$%&*()}{@#~?><>,|=_+¬-]/',$username)){
			echo "<script>if(confirm('Please do not use special characters!')){document.location.href='individual.php'};</script>";
			return;
		}
		
		
		if(mysqli_num_rows($query_username) || mysqli_num_rows($query_username2)){
			echo "<script>if(confirm('There is already a user with that name!')){document.location.href='individual.php'};</script>";
			return;
		}
		
		
		if($password != $password_confirm){
			echo "<script>if(confirm('Passwords do not match!')){document.location.href='individual.php'};</script>";
			return;
		}
		
		
		if(!filter_var($email, FILTER_VALIDATE_EMAIL) || $email == "") {
			echo "<script>if(confirm('This email is not valid!')){document.location.href='individual.php'};</script>";
			return;
		}
		
		if(mysqli_num_rows($query_email)) {
			echo "<script>if(confirm('That email is already in use!')){document.location.href='individual.php'};</script>";
			return;
		}
		...
		if($result){
			header("Location: aboutsignin.php");
		} else {
			echo "<script>if(confirm('Cannot add to database')){document.location.href='individual.php'};</script>";
			return;
		}
...
<div class="contact-w3l">
		<div class="container">
			<!-- tittle heading -->
			<h3 class="tittle-w3l">Individual Sign Up<span class="heading-style"> <em></em> <em></em> <em></em> </span> </h3>
			<!-- //tittle heading -->
			<!-- contact -->
					...
					<div class="contact-form wthree">
						<form action="individual.php" method="post" enctype="multipart/form-data">
							...
							<div class="">
								...
								<input class="phone" type="text" name="phone" placeholder="Phone" required="">
							</div>
